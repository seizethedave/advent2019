...
type distanceFunc func(distance int)

func travel(x, y, dx, dy, distance, traveled int, dist map[string]int,
	markDist bool, intersectCallback distanceFunc) (int, int, int) {
	...
	for i := 0; i < distance; i++ {
		...
		if stored, hit := dist[key]; hit && intersectCallback != nil {
			intersectCallback(stored + traveled)
		} else if markDist {
			dist[key] = traveled
		}
	}
...
func evalString(input string, dist map[string]int, markDist bool, intersectCallback distanceFunc) {
	var x, y, traveled int
	...
	for _, instr := range strings.Split(input, ",") {
		...
		x, y, traveled = travel(x, y, dx, dy, num, traveled, dist, markDist, intersectCallback)
	}
}
...
func main() {
	...
	evalString(input1, dist, true, nil)

	minDist := -1

	evalString(input2, dist, false, func(distance int) {
